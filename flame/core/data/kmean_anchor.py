...
def cas_ratio(box, cluster):
    ...
    ratios = np.concatenate(
        [ratios_of_box_cluster, ratios_of_cluster_box], axis = -1
    )
...
def kmeans(box, k):
    row = box.shape[0]  # how many boxes are taken out
    distance = np.empty((row, k))  # the position of each point in each box
    last_clu = np.zeros((row,))  # the last cluster position
    np.random.seed()
    cluster = box[np.random.choice(row, k, replace=False)]  # 5 randomly selected as cluster centers.

    iter = 0
    while True:
        # calculate the aspect ratio of the current box and the prior box.
        for i in range(row):
...
if __name__ == '__main__':
    ...
    #   载入数据集，可以使用VOC的xml
    #-------------------------------------------------------------#
    path        = '/media/phungpx/WORKSPACE/PROJECTS/PHUNGPX/object_detection_pytorch/DATASET/PASCALVOC2012/Annotations'
